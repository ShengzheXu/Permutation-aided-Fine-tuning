...
from be_great.great_dataset import GReaTDataset, GReaTDataCollator, NonPermuteDataset
...
class GReaT:
    ...
    def fit(
        ...
        print("Done self.conditional_col_dist: ", self.conditional_col_dist)
    
        # Convert DataFrame into HuggingFace dataset object
        logging.info("Convert data into HuggingFace dataset object...")
        if having_order:
            great_ds = NonPermuteDataset.from_pandas(df)
        else:
            great_ds = GReaTDataset.from_pandas(df)
        print("Done great_ds: ", great_ds)
        
        great_ds.set_tokenizer(self.tokenizer)
        # pdb.set_trace()
        # Set training hyperparameters
        print('self.experiment_dir: ', self.experiment_dir)
        print('Create GReaT Trainer...')
        ...
        # tt = great_trainer.get_train_dataloader()
        # # train_features, train_labels = next(iter(tt))
        # for jj in range(5):
        #     what = next(iter(tt))
        #     for k, v in what.items():
        #         print(k, v.shape)
        #         np.savetxt(f'see_examples/row{jj}_{k}.txt', v.numpy(), fmt='%d')
        # what to json file
        # with open('what.json', 'w') as f:
        #     json.dump(what, f)
    ...
    def sample(
        ...
        """
        # print("start_col: ", start_col, type(start_col))
        # print("start_col_dist: ", start_col_dist, type(start_col_dist))
        great_start = self._get_start_sampler(start_col, start_col_dist, choice=starter)
        # print("great_start: ", great_start)
        # input()
        ...
        with tqdm(total=n_samples) as pbar:
            ...
            try:
                while n_samples > already_generated:
                    ...
                    print("step text_data: ", text_data)
                    # input()
    ...
    def _get_start_sampler(
        ...
        if choice is not None:
            if choice == 'Empty':
                return EmptyStart(self.tokenizer, self.columns)
            elif choice == 'Random':
        ...
        if isinstance(start_col_dist, dict):
            print('2-start_col_dist', start_col_dist, type(start_col_dist))
